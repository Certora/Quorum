...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--prompt_template', type=Path, default=Path(__file__).parent / 'ipfs_validation_prompt.txt',
                        help='Optional other template for prompting the LLM.')
    
    args = parser.parse_args()
    if not args.prompt_template.exists():
        raise argparse.ArgumentError(f'Could not find prompts at {args.prompt_template}.')
